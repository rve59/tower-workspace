...
# Next 5 filers to cross the 50M threshold
NEXT_IDS = ["6155459", "6172087", "5978947", "6069192", "5910835"]

def ingest_more():
    lookup = pl.read_parquet(LOOKUP_PATH)
    filings = lookup.filter(
        (pl.col("company_id").is_in(NEXT_IDS)) & 
        (pl.col("year_quarter") == "2025Q1")
    ).to_dicts()
    
    print(f"Ingesting {len(filings)} more filings...")
